    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted">{data ? `${data.total} sinistro(s) encontrado(s).` : 'Carregando…'}</p>
        </div>
        <Link
          href="/sinistros/novo"
      ...
      <div className="flex flex-wrap gap-3">
        <input
          ...
          placeholder="Buscar por nº, apólice, placa, segurado…"
          ...
          <tbody>
            ...
            {data?.items.map((claim) => (
              <tr key={claim.id} className="border-b border-border last:border-0 hover:bg-surface-hover">
                ...
                <td className="px-4 py-2 text-ink">{claim.insurerName ?? '—'}</td>
